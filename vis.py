...
def plot_velocity(guess, output, target, path, vmin=None, vmax=None):
    fig, ax = plt.subplots(3, 1, figsize=(11, 16))
    if vmin is None or vmax is None:
        vmax, vmin = np.max(target), np.min(target)
    im = ax[0].matshow(guess, cmap=rainbow_cmap, vmin=vmin, vmax=vmax)
    ax[0].set_title('Initial guess', y=1.08)
    ax[1].matshow(output, cmap=rainbow_cmap, vmin=vmin, vmax=vmax)
    ax[1].set_title('Prediction', y=1.08)
    ax[2].matshow(target, cmap=rainbow_cmap, vmin=vmin, vmax=vmax)
    ax[2].set_title('Ground Truth', y=1.08)
    ...
    for axis in ax:
        ...
        axis.set_xticks(range(0, 384, 50))
        axis.set_xticklabels(range(0, 3840, 500))
        axis.set_yticks(range(0, 134, 20))
        axis.set_yticklabels(range(0, 1340, 200))
