...
def calc_grad_penalty(discriminator, real, fake):

    lambda_gp = 10.
   
    alpha = torch.rand(real.shape[0], 1, 1, 1)
    alpha = alpha.expand(real.size()).to(real.device)
    ...
    gradient_penalty = ((gradients.norm(2, dim=(1, 2, 3)) - 1) ** 2).mean() * lambda_gp
    ...
    try:
        ...
        while True:
            ...
            for _ in range(critic_iters):
                ...
                data, real = next(dataloader)

                optimizer_disc.zero_grad()

                guess = torch.clone(real).data.numpy()
                for _ in range(2): guess = gaussian_filter(guess, (0, 0, 5, 5))
                guess = torch.from_numpy(guess).to(device)

                data, real = data.to(device), real.to(device)

                with torch.no_grad(): fake  = generator(data, guess)
                
                preds_real = discriminator(real)
                preds_fake = discriminator(fake)

                loss, loss_gan, loss_gp = criterion_disc(discriminator, real, fake, preds_real, preds_fake)
                ...
                metric_logger.update(
                    ...
                    wasserstein_loss_discriminator=loss_gan_val, 
                    gradient_penalty_discriminator=loss_gp_val, 
                    lr_discriminator=optimizer_gen.param_groups[0]['lr']
                )

                metric_logger.meters['samples/s'].update(batch_size / (time.time() - start_time))
                
                if writer:
                    writer.add_scalar('discriminator_loss', loss_val, step)
                    writer.add_scalar('wasserstein_loss_discriminator', loss_gan_val, step)
                    writer.add_scalar('gradient_penalty_discriminator', loss_gp_val, step)
            ...
            fake        = generator(data, guess)
            preds_fake  = discriminator(fake)

            loss, loss_gan, loss_content = criterion_gen(real, fake, preds_fake)
            ...
            loss_val         = loss.item()
            loss_gan_val     = loss_gan.item()
            loss_content_val = loss_content.item()
            batch_size       = data.shape[0]

            print(f"Content Loss: {loss_content_val:.8f}")

            metric_logger.update(
                generator_loss=loss_val,
                wasserstein_loss_generator=loss_gan_val, 
                content_loss_generator=loss_content_val, 
                lr_generator=optimizer_gen.param_groups[0]['lr']
            )

            metric_logger.meters['samples/s'].update(batch_size / (time.time() - start_time))
            
            if writer:
                writer.add_scalar('generator_loss', loss_val, step)
                writer.add_scalar('wasserstein_loss_generator', loss_gan_val, step)
                writer.add_scalar('content_loss_generator', loss_content_val, step)
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for data, real in metric_logger.log_every(dataloader, 20, header):
            ...
            loss_g, _, loss_content = criterion_gen(real, fake, preds_fake)
            # loss_d, _, loss_gp      = criterion_disc(discriminator, real, fake, preds_real, preds_fake)
    ...
    if writer:
        writer.add_scalar('loss_g', metric_logger.loss_g.global_avg, step)
        # writer.add_scalar('loss_d', metric_logger.loss_d.global_avg, step)
        writer.add_scalar('loss_content', metric_logger.loss_content.global_avg, step)
    
    return metric_logger.loss_content.global_avg
...
def main(args):
    ...
    generator = unet.UNet().to(device)
  
    # generator = network.InversionNet(
    ...
    #    generator.load_state_dict(torch.load(os.path.join(args.output_path, 'generator.pth'))['model'])
    ...
    def criterion_gen(real, fake, preds_fake):

        loss_gan     = -torch.mean(preds_fake)
        loss_content = 500. * (l1loss(real, fake) + l2loss(real, fake) + 0.1 * ssimloss(real, fake))
        loss         = loss_gan + loss_content
        return loss, loss_gan, loss_content 

    def criterion_disc(discriminator, real, fake, preds_real, preds_fake):
        
        loss_gan = torch.mean(preds_fake) - torch.mean(preds_real)
        loss_gp  = calc_grad_penalty(discriminator, real, fake)
        loss     = loss_gan + loss_gp

        return loss, loss_gan, loss_gp
    ...
    optimizer_gen  = torch.optim.AdamW(
        generator.parameters(), lr=lr, betas=(0.9, 0.999), weight_decay=args.weight_decay)
    optimizer_disc = torch.optim.AdamW(
        discriminator.parameters(), lr=lr, betas=(0.9, 0.999), weight_decay=args.weight_decay)

    # Convert scheduler to be per iteration instead of per epoch
    warmup_iters = args.lr_warmup_epochs * len(dataloader_train)
    lr_milestones = [len(dataloader_train) * m for m in args.lr_milestones]
    
    lr_scheduler_gen = WarmupMultiStepLR(
        optimizer_gen, 
    ...
    lr_scheduler_disc = WarmupMultiStepLR(
        optimizer_disc, 
        milestones=lr_milestones, 
        gamma=args.lr_gamma,
        warmup_iters=warmup_iters, 
    ...
    if args.resume:
        ...
        lr_scheduler_gen.load_state_dict(checkpoint['lr_scheduler_gen'])
        lr_scheduler_disc.load_state_dict(checkpoint['lr_scheduler_disc'])
        args.start_epoch = checkpoint['epoch'] + 1
        step = checkpoint['step']
        lr_scheduler_gen.milestones=lr_milestones
        lr_scheduler_disc.milestones=lr_milestones
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(
            ...
            criterion_gen, criterion_disc, 
            optimizer_gen, optimizer_disc, 
            lr_scheduler_gen, lr_scheduler_disc, 
            dataloader_train,
            device, epoch, 
            args.print_freq, train_writer
        )
        
        loss = evaluate(
            generator, discriminator, 
            criterion_gen, criterion_disc, 
            dataloader_valid, device, val_writer
        )
